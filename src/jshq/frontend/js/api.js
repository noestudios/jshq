...
export const api = {
  ...
  listNextSteps: () => request("GET", "/api/next-steps"),
  createNextStep: (body) => request("POST", "/api/next-steps", body),
  patchNextStep: (id, body) => request("PATCH", `/api/next-steps/${id}`, body),
  deleteNextStep: (id) => request("DELETE", `/api/next-steps/${id}`),
